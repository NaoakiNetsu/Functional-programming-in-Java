...
## 余談_Spring Framework でDIされたインスタンスはSingleton

デフォルトではDIされたインスタンスはシングルトンである。  
起動時にインスタンスが生成され、以降使いまわされる。

★参考文献  
http://atuweb.net/201602_spring-what-i-happend/

Spring Frameworkではインスタンスの生成をDIを用いてやるという思想である。  

★参考文献  
http://dev.classmethod.jp/server-side/java/spring-firstcontact/  
「Spring Framework4 プログラミング入門」p.228
